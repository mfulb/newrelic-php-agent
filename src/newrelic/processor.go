...
func ConnectApplication(args *ConnectArgs) ConnectAttempt {
	...
	log.Debugf("ConnectApplication(): preconnect rawreply = %+v", rep.RawReply)

	if nil != rep.RawReply.Err {
		log.Debugf("ConnectApplication(): preconnect rep.RawReply.Err != nil (%s)", rep.RawReply.Err)
...
func (p *Processor) processConnectAttempt(rep ConnectAttempt) {
	...
	log.Debugf("processConnectAttempt(): rep = %+v", rep)
...
func harvestPayload(p PayloadCreator, args *harvestArgs) {
	...
	log.Debugf("harvestPayload: reply = %v", reply)
